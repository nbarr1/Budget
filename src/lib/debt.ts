...
export function simulateDebtPayoff(debts: Debt[], strategy: Strategy, extraMonthlyCents=0, lumpSumCents=0) {
  ...
  while(rows.some(d=>d.balanceCents>0) && month<600){ month++; const active=rows.filter(d=>d.balanceCents>0); for(const d of active){ const interest=Math.round(d.balanceCents*(d.aprBps/10000)/12); d.balanceCents+=interest; totalInterest+=interest; }
    let pool=active.reduce((s,d)=>s+d.minimumPaymentCents,0)+extraMonthlyCents; rows.sort((a,b)=>strategy==='snowball'?a.balanceCents-b.balanceCents:b.aprBps-a.aprBps);
