import { addDays, addMonths, compareAsc, differenceInCalendarDays, differenceInCalendarMonths, endOfMonth, formatISO, isAfter, isBefore, isEqual, max, parseISO, setDate, startOfDay } from 'date-fns';
...
function onOrAfter(date: Date, boundary: Date) { return isEqual(date, boundary) || isAfter(date, boundary); }
function onOrBefore(date: Date, boundary: Date) { return isEqual(date, boundary) || isBefore(date, boundary); }
function pushIfInRange(out: Occurrence[], entry: Entry, date: Date, from: Date, to: Date) {
  ...
  const start = startOfDay(entry.startDate); const end = entry.endDate ? startOfDay(entry.endDate) : null;
  if (onOrAfter(d, from) && onOrBefore(d, to) && onOrAfter(d, start) && (!end || onOrBefore(d, end)) && !skips.has(dayKey(d))) {
...
export function generateOccurrences(entry: Entry, fromInput: Date, toInput: Date): Occurrence[] {
  ...
  if (['MONTHLY','QUARTERLY','ANNUAL'].includes(s.frequency)) {
    ...
    const diffMonths = differenceInCalendarMonths(effectiveFrom, start);
    const intervals = Math.max(0, Math.floor(diffMonths / step) - 1);
    for (let cursor = addMonths(new Date(start.getFullYear(), start.getMonth(), 1), intervals * step); !isAfter(cursor, to) && !isAfter(cursor, end); cursor = addMonths(cursor, step)) pushIfInRange(out, entry, resolveMonthDay(cursor, s.dayOfMonth ?? start.getDate()), from, to);
    return out.sort((a,b)=>compareAsc(a.date,b.date));
  }
  const days = s.frequency === 'WEEKLY' ? 7 : s.frequency === 'BIWEEKLY' ? 14 : Math.max(1, s.interval ?? 1);
  const diffDays = differenceInCalendarDays(effectiveFrom, start);
  const intervals = Math.max(0, Math.floor(diffDays / days) - 1);
  for (let cursor = addDays(start, intervals * days); !isAfter(cursor, to) && !isAfter(cursor, end); cursor = addDays(cursor, days)) pushIfInRange(out, entry, cursor, from, to);
  return out.sort((a,b)=>compareAsc(a.date,b.date));
}
