...
export function projectCashFlow(accounts: Account[], entries: Entry[], from: Date, to: Date, accountId?: string): DayProjection[] {
  let bal = accounts.filter(a=>!accountId || a.id===accountId).reduce((s,a)=>s+a.balanceCents,0);
  const events = getOccurrences(entries, from, to, accountId).map(e => ({ ...e, dateKey: formatISO(e.date, { representation: 'date' }) }));
  const days: DayProjection[]=[];
  for(let d=startOfDay(from); d<=startOfDay(to); d=addDays(d,1)){
    const key=formatISO(d,{representation:'date'});
    const todays=events.filter(e=>e.dateKey===key);
    const income=todays.filter(e=>e.type==='INCOME').reduce((s,e)=>s+e.amountCents,0);
    const expense=todays.filter(e=>e.type==='EXPENSE').reduce((s,e)=>s+e.amountCents,0);
    const start=bal;
    bal += income - expense;
    days.push({date:d,startingBalanceCents:start,incomeCents:income,expenseCents:expense,endingBalanceCents:bal,events:todays});
  }
  return days;
}
