...
export function computeSafeToSpend(accounts: Account[], entries: Entry[], bufferCents: number, today: Date, horizonDays = 90, accountId?: string) {
  const to = addUtcDays(today, horizonDays); const occ = getOccurrences(entries, today, to, accountId); const nextPayday = occ.find(o=>o.type==='INCOME' && o.category.toLowerCase() === 'job' && compareUtcDays(o.date, today) > 0)?.date ?? null; const until = nextPayday ?? to;
